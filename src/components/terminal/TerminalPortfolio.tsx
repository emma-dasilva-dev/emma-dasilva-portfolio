...
export function TerminalPortfolio({ locale }: { locale: Locale }) {
  ...
  const copy = locale === "en" ? {
    welcome: "Welcome to my interactive portfolio.", hint: "Type a command below or use one of the shortcuts.", role: "Cybersecurity & Computer Engineering student", focus: "linux · networking · web security", secondary: "software engineering", available: "available commands:", unknown: "command not found", tip: "type a command, click a shortcut, or use ↑ / ↓ to revisit command history.", stackIntro: "tools and technologies currently in use:", githubIntro: "My GitHub contains my repositories, experiments and ongoing development work.", githubLink: "view GitHub profile", listen: "listen to intro", stop: "stop audio", voiceLabel: "Listen to Emma's introduction", voiceIntro: "Hi, I'm Emma. I'm a cybersecurity and computer engineering student focused on Linux, networking, web security, and software engineering. I like understanding how systems work, building practical projects, and learning by solving technical problems.",
  ...
  return <main className={styles.desktop} onClick={() => inputRef.current?.focus()}>
    ...
    <section className={styles.terminalWindow} aria-label="Ubuntu terminal portfolio">
      <header className={styles.terminalHeader}><div className={styles.lights}><i/><i/><i/></div><span>emma@portfolio: ~</span><a className={styles.languageSwitch} href={`/${locale === "en" ? "fr" : "en"}`} aria-label={locale === "en" ? "Passer en français" : "Switch to English"} onClick={e => e.stopPropagation()}><span className={locale === "en" ? styles.languageActive : undefined}>EN</span><span className={locale === "fr" ? styles.languageActive : undefined}>FR</span></a></header>
      <div className={styles.terminalBody}>
        <div className={styles.boot}><div className={styles.hero}><div><p className={styles.welcomeCommand}><Prompt/> welcome</p><pre className={styles.ascii}>EMMA DA SILVA</pre><h1>{copy.role}</h1><p className={styles.specialization}>{copy.focus}<span> · {copy.secondary}</span></p><div className={styles.rule}/><p><strong>{copy.welcome}</strong><br/>{copy.hint}</p><dl><dt>user</dt><dd>emma</dd><dt>host</dt><dd>portfolio</dd><dt>os</dt><dd>ubuntu</dd><dt>location</dt><dd>cotonou, benin</dd><dt>focus</dt><dd>{copy.focus}</dd></dl></div><div className={styles.voiceIntro} aria-label={copy.voiceLabel}><div className={`${styles.voiceOrb} ${isSpeaking ? styles.voiceOrbSpeaking : ""}`} aria-hidden="true"><span/><span/><span/></div><span className={styles.orbShadow}/><button type="button" className={styles.voiceButton} onClick={e => { e.stopPropagation(); toggleIntroduction(); }} aria-pressed={isSpeaking}>{isSpeaking ? copy.stop : copy.listen}</button><small>{locale === "en" ? "~20 sec" : "~20 s"}</small></div></div>
